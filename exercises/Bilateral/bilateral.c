...
int main(int argc, char *argv[])
{
  ...
  char *source = loadProgram("bilateral.cl");
  cl_program program =
  ...
  err = clSetKernelArg(kernel, 2, sizeof(float), &sigmaDomain);
  checkError(err, "setting argument 2");
  err = clSetKernelArg(kernel, 3, sizeof(float), &sigmaRange);
  checkError(err, "setting argument 3");
  err = clSetKernelArg(kernel, 4, sizeof(cl_uint), &radius);
  checkError(err, "setting argument 4");
