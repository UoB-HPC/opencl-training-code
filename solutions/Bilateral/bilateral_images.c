...
int main(int argc, char *argv[])
{
  ...
  clGetDeviceInfo(device, CL_DEVICE_IMAGE_SUPPORT,
                  sizeof(cl_bool), &supportsImages, NULL);
  if (!supportsImages)
  {
     printf("\nDevice doesn't support images!\n\n");
     return 1;
  }
  ...
  char *source = loadProgram("bilateral_images.cl");
  cl_program program =
  ...
  cl_mem input = clCreateImage(context, CL_MEM_READ_ONLY,
                               &format, &desc, NULL, &err);
  checkError(err, "creating input image");
  cl_mem output = clCreateImage(context, CL_MEM_WRITE_ONLY,
                                &format, &desc, NULL, &err);
  checkError(err, "creating output image");

  // Write image to device
  size_t origin[3] = {0, 0, 0};
  size_t region[3] = {width, height, 1};
  err = clEnqueueWriteImage(queue, input, CL_TRUE, origin, region, 0, 0,
